    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Cumulative Flow Diagram (CFD) & Burnup Section */}
          <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-xs space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-gray-900 text-base flex items-center gap-2">
                ...
                <p className="text-xs text-gray-400">Work in progress progression across workflow stages over time</p>
              </div>
            ...
            {/* CFD Visual Stacked Representation */}
            <div className="pt-2 space-y-2">
              <div className="flex justify-between items-center text-xs font-semibold text-gray-500">
                <span>Stage Work Distribution</span>
                <span>Total Work Items: {data.totalTasks}</span>
              </div>
              <div className="w-full h-8 bg-gray-100 rounded-xl overflow-hidden flex shadow-inner">
                <div
                  className="bg-slate-400 h-full flex items-center justify-center text-[10px] font-bold text-white transition-all"
                  style={{ width: `${data.totalTasks > 0 ? (data.statusCounts.Todo / data.totalTasks) * 100 : 25}%` }}
                  title={`Todo: ${data.statusCounts.Todo}`}
                >
                  {data.statusCounts.Todo > 0 && `Todo (${data.statusCounts.Todo})`}
                </div>
                <div
                  className="bg-amber-500 h-full flex items-center justify-center text-[10px] font-bold text-white transition-all"
                  style={{ width: `${data.totalTasks > 0 ? (data.statusCounts.Doing / data.totalTasks) * 100 : 25}%` }}
                  title={`Doing: ${data.statusCounts.Doing}`}
                >
                  {data.statusCounts.Doing > 0 && `Doing (${data.statusCounts.Doing})`}
                </div>
                <div
                  className="bg-indigo-500 h-full flex items-center justify-center text-[10px] font-bold text-white transition-all"
                  style={{ width: `${data.totalTasks > 0 ? (data.statusCounts.Testing / data.totalTasks) * 100 : 25}%` }}
                  title={`Testing: ${data.statusCounts.Testing}`}
                >
                  {data.statusCounts.Testing > 0 && `QA (${data.statusCounts.Testing})`}
                </div>
                <div
                  className="bg-emerald-500 h-full flex items-center justify-center text-[10px] font-bold text-white transition-all"
                  style={{ width: `${data.totalTasks > 0 ? (data.statusCounts.Done / data.totalTasks) * 100 : 25}%` }}
                  title={`Done: ${data.statusCounts.Done}`}
                >
                  {data.statusCounts.Done > 0 && `Done (${data.statusCounts.Done})`}
                </div>
              </div>

              <div className="flex items-center justify-center gap-6 pt-2 text-xs font-semibold">
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-slate-400"></span> Todo</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-amber-500"></span> Doing</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-indigo-500"></span> Testing</span>
                <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-emerald-500"></span> Done</span>
              </div>
            </div>
          </div>
